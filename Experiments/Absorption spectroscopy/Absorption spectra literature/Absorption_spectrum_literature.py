...
COLORS = [my_cmap(i) for i in range(my_cmap.N)]
    ...
    for i,defect in enumerate(DICO):
        for j,dicpeak in enumerate(DICO[defect]):
            ...
            if dicpeak["FWHM"]==1e-6:
                lstyle="--"
            if j==0:
                LEGENDCOLORS += [plt.Line2D([0], [0], color=COLORS[i])]
                sources=""
            ...
            ax.semilogy(*plotlists_peak(dicpeak["peak"],dicpeak["FWHM"],normalizemax=False),color=COLORS[i],linestyle=lstyle)
